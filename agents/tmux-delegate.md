...
- It is a follow-up task in the same repo/domain as an existing worker with clean context: prefer sending to that worker (after `result init`) over starting a new one.
...
Boundary: a single already-known command (test, build, lint) runs inline even when it touches many files — the delegate thresholds above target multi-step work that needs reasoning, not one known command.

If delegating, also decide the relationship: one-shot task, or a teammate (follow-up dialogue with the same worker expected — task series, review loop)? Teammate → interactive `start` (no `--headless`), do not `stop` between tasks, and reuse via the protocol in `skills/tmux-agent-tools/references/multi-agent.md#persistent-teammates-worker-reuse`.
...
Shape the prompt body itself with the `delegation-templates` skill (GOAL / ACCEPTANCE / REPORT + common footer + tmux worker addendum) — a delegation without objective acceptance criteria is a guess you outsourced.
...
```sh
claude-tmux start --exact --task-shape <bounded|series|review-loop|exploratory> <safe-name> <repo-dir> '<worker-prompt>'
codex-tmux start --exact --task-shape <bounded|series|review-loop|exploratory> <safe-name> <repo-dir> '<worker-prompt>'
```

For generic CLIs:

```sh
agent-tmux <cli> start --exact --task-shape <bounded|series|review-loop|exploratory> <safe-name> <repo-dir> '<worker-prompt>'
```

Always pass `--task-shape` with the shape you decided above: `bounded` for one-shots, `series` / `review-loop` for teammate relationships, `exploratory` when the scope is open-ended. This records the decision in launch metadata for fleet telemetry (`stats`).
