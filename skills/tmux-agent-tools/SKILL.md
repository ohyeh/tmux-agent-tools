...
- **Multiple workers, need first/all/quorum completions?** One blocking call: `codex-tmux watch --any|--all|--count <n> --timeout <s> --json <n1> <n2> …` — do **not** write a shell polling loop. Parse the JSON `agents[].done/reason` for the winner/quorum, then `codex-tmux result --json <winner>`. Note: `reason:exited` is reliable only within the same wrapper family (prefix-tied liveness); for **mixed-engine fleets** trust only `reason:result_updated` (fully engine-agnostic) or use `tmux-agent-sessions` — a codex-tmux watch can falsely report an agy or claude session as `exited` because it cannot see the foreign-prefix tmux session.
