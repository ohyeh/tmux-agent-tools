...
- `mods/tmux-agent` 0.7.5 + `agent-tmux` — fixes the astra review of 0.7.4 (`VERDICT: BLOCK`, six findings plus four pre-existing). **Commit evidence**: a `commit` that is not a string (`{"toString":null}` threw inside `String()` and the whole success was never delivered) or is `''` is delivered once as `NOT verified`; the sha must be a `commit` object (`git cat-file -t`; an annotated tag id resolved `^{commit}` and passed as "verified"), and new work on the dispatch base — `assign` and every `tell` record `base` (`git rev-parse HEAD` of `dir`, read before the worker starts) and the claim must satisfy `merge-base --is-ancestor <base> <sha>` with `sha ≠ base`, so an old commit no longer passes. No base (not a repo, or a pre-0.7.5 record) says `commit object exists; no dispatch base recorded`. All commit checks of one pass share a 4 s budget; what does not fit waits for the next tick instead of reading as "no such commit" (20 × 2 s could hold a tick for 40 s). **Launch-failed is provisional**: the notice is acked as `<name>@<since>#launch`, so a real result of the same episode — the CLI took the brief without showing it, as claude-fable-gate did from its session picker on 2026-09-24 — is still delivered once, and a terminal result outranks the receipt in `collect` and the panel. **Stalls**: `agent-tmux status --json` now reports `blocked_reason: quota_exhausted | login_required` with `blocked_evidence` (the line) and a diagnostic, read from the CLI's last 10 output lines above the prompt with CLI-shaped phrases only (the live codex banner `■ You’ve hit your usage limit` sits above `›`, outside the prompt area, so status answered `null` all night); new `test-runtime-blocker-status-smoke` (10 pass: live banner, a worker's own "rate limit" prose, a banner pushed into history, invalid api key, a pane with no prompt boundary — claude/agy/cursor shapes UNCONFIRMED). The mod drops its own `BLOCKER_RE` (it matched "Implemented rate limit handling" and missed "invalid api key") and reads the wrapper's reason: after 2 min unchanged it marks the worker `stalled` and **wakes the session once per episode** — two codex workers sat on a usage limit for an hour while the lead waited. A dialog reason clears the episode's stall so panel, log and `$.tmux.stalled()` agree. **Result validation**: `result --validate` now checks `commit` (40-hex lowercase, or omit; `null` in the file is invalid, readers still tolerate it). **Tests**: the budget test exercises the budget seam directly (another session's key over 3 MiB) instead of scanning 65k directories — it ran at the 5 s timeout on 0.7.4 and its baseline (22 ms now). Docs: README, types, SKILL.md, contracts.md, schemas, using-tmux-agent-tools.
