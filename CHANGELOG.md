...
- mod: a `launch-failed` notice whose failed step is `confirm-processing` now says the brief was sent, the step is judged from the pane, the worker may be working, and to peek before dispatching again (a later result is still delivered). Live 2026-09-25: a cursor reviewer reading 22k tokens was reported launch-failed. Mod tests 141 pass (140/1 without the change).
