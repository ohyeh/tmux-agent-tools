...
- `claude-tmux start --dry-run` and `codex-tmux start --dry-run` perform pre-flight checks without creating a tmux session or launching the CLI (issue #110). Emits JSON `{schema_version: 1, tool, name, directory, ok: bool, checks: [...]}` with per-check `{name, status: pass|fail|skip, detail}`. Checks covered: workdir_exists, tmux_binary, agent_cli_binary, session_not_in_use (only with `--exact`), sentinel_path (absolute + writable parent + not pre-existing), on_exit_pairing (`--on-exit` requires `--sentinel`), transcript_path (absolute + writable + not pre-existing). Exit 0 on all-pass, exit 2 on any failure. Side-effect free: dry-run never calls `tmux new-session`. Scope is `start` only; `resume`, `start-ssh`, `tmux-agent-dialogue`, and `--check-script` remain followups. `scripts/test-dry-run-smoke`: 36 sub-assertions across both wrappers covering happy path, missing workdir, pre-existing sentinel, orphan `--on-exit`, relative transcript, the "no side effects" invariant, plus 4 regression cases from gemini review (cross-tool env fallback prevention, transcript writability without creating the file).
- `--on-start <shell-cmd>` flag on `claude-tmux` and `codex-tmux` `start` / `resume` (issue #101a). After `tmux new-session` returns, the wrapper fires the hook in a detached subshell with `TMUX_AGENT_NAME` and `TMUX_AGENT_SESSION` exported. Hook stdout/stderr captured to `<sentinel>.hook.log` (when `--sentinel` is set) or `$TMUX_AGENT_DIR/<name>/hook.log`. Non-zero hook exit is logged via `[on-start exit] <code>` but never fails the agent. `scripts/test-on-start-smoke` covers basic invocation, missing-value rejection, sentinel-log routing, failing-hook isolation, and "no flag → no log file" defaults with 18 sub-assertions.
