...
- `tmux-agent-commander panel <session>` prints one line of workers that session owns: running `▶`, a terminal result as `✓ success` or `✗ <status>`, then the three newest `dispatch.delivered.json` rows as `✓ delivered`. `start` installs that line on the session only (`status 2`, `status-format[1]`, `status-interval 2`); `--no-panel` skips it. A split pane is not used, because `agent-tmux -t <session>` types into the active pane. `--width N` drops whole entries from the end and appends `+N more`. `scripts/test-commander-smoke` 69 pass.
