...
Cloudflare Worker (stateless router, ~50 lines)
      ...
      ▼
Durable Object: RoostDO (roost) (one per team)
...
## 4. Durable Object: RoostDO (roost)
...
3. **Route to DO**: forward validated request to `RoostDO` stub for the team.
...
```text
atrium/                         # Cloudflare project root (separate from agent-tmux repo, or subfolder)
  wrangler.toml                 # Worker + DO binding config
  src/
    index.ts                    # Worker router (~50 lines)
    roost-do.ts                  # RoostDO class (~150 lines)
  package.json
  tsconfig.json
```
...
The project lives in a subdirectory `atrium/` under the repo root (or a separate repo at user's preference). It is **not part of the agent-tmux formula tarball**; it is deployed separately via `wrangler deploy`.
