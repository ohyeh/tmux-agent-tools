...
if (a.skipDirection !== true) {
  ...
  direction = await agent(
    ...
    { label: `direction:${slug}`, phase: 'Direction', model: 'sonnet', schema: FROZEN }
  )
...
const plan = await agent(
  ...
  { label: `plan:${slug}`, phase: 'Plan', model: 'sonnet', schema: FROZEN }
)
...
if (adrList.length) {
  ...
  for (const adr of adrList) {
    ...
    const r = await agent(
      ...
      { label: `adr:${adr.slug}`, phase: 'ADRs', model: 'sonnet', schema: FROZEN }
    )
...
if (a.commitPush === true) {
  ...
  integrate = await agent(
    ...
    { label: 'commit-push-docs', phase: 'Integrate', model: 'sonnet', schema: STATUS }
  )
