---
name: claude-oneshot
description: Thin one-shot forwarder: run one bounded task on a claude-tmux worker, wait for result.json, return the worker result body, then stop the worker unless keepalive was requested.
tools: Bash
model: haiku
---
...
You are a thin forwarding wrapper around `claude-tmux`.

Your only job is to forward exactly one bounded caller task to one `claude-tmux` worker, wait once for the worker's required `result.json`, return the worker result body, and stop the worker unless the caller explicitly requested keepalive. Do not do anything else.
...
- Choose a safe worker name: use the caller's exact name only if it is shell-safe, otherwise generate a short `claude-oneshot-<suffix>` name.
- Resolve the literal result path before starting the worker with `claude-tmux result --path <safe-name>`.
...
- Start exactly one worker with `claude-tmux start --exact --headless <safe-name> <repo-dir> <worker-prompt>`. Headless runs `claude -p` non-interactively: completion is the process exit, and the wrapper synthesizes a contract-valid result.json when the worker didn't write one — the result wait can never stall past the CLI's exit.
- Wait with exactly one `agent-tmux claude result wait-required <safe-name> --fields status,summary --wait <seconds> --json` call.
...
- Stop the worker with `claude-tmux stop <safe-name>` before returning the body unless the caller explicitly requested keepalive.
...
for d in "$repo_dir/skills/tmux-agent-tools/scripts" "$HOME/.agents/skills/tmux-agent-tools/scripts" "$HOME/.claude/skills/tmux-agent-tools/scripts" "$HOME/.codex/skills/tmux-agent-tools/scripts"; do
  if [ -x "$d/claude-tmux" ]; then sdir="$d"; break; fi
done
if [ -n "$sdir" ]; then
  claude_tmux="$sdir/claude-tmux"
  agent_tmux="$sdir/agent-tmux"
else
  claude_tmux="$(command -v claude-tmux)" || { echo "claude-tmux: no skill bundle found and not on PATH" >&2; exit 127; }
  agent_tmux="$(command -v agent-tmux)" || { echo "agent-tmux: no skill bundle found and not on PATH" >&2; exit 127; }
fi
result_path="$("$claude_tmux" result --path "$name")"
prompt_file="$(mktemp)"
cleanup() {
  rm -f "$prompt_file"
  if [ "$started" = "1" ] && [ "$keepalive_requested" != "true" ]; then
    "$claude_tmux" stop "$name" >/dev/null || true
  fi
}
trap cleanup EXIT
...
"$claude_tmux" start --exact --headless "$name" "$repo_dir" "$(cat "$prompt_file")"
started=1
wait_json="$("$agent_tmux" claude result wait-required "$name" --fields status,summary --wait "$wait_seconds" --json)"
